...
-- Wrap everything in a DO block so the seed is CI-safe.
-- In CI there are no auth users, so the FK on profiles.id would fail.
-- We check for the first placeholder user and skip the entire seed if missing.

DO $$
BEGIN
  -- Guard: skip seed if placeholder auth users don't exist
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE id = 'aaaaaaaa-0000-0000-0000-000000000001') THEN
    RAISE NOTICE 'Seed skipped: placeholder auth users not found (expected in CI)';
    RETURN;
  END IF;
