...
export async function confirmVendor(serviceId: string) {
  ...
  // Get service and verify permission
  ...
  // Check permission via RPC or helper - use service client to verify
  // The actual permission check is done by requiring the caller has the right profile
  // For admin, this is always allowed. For client portal, we check in the component.
...
export async function updateScheduleItem(itemId: string, formData: FormData) {
  ...
  await logActivity({
    ...
    summary: `Updated schedule item "${item.title}"`,
  });
